...
export function formatEvidence(ruleId: string, evidence: EvidenceMap | null | undefined): string {
  ...
  switch (ruleId) {
    case "retry_storm": {
      ...
      const threshold = evidence.threshold as number | undefined;
      if (count === undefined) return "";
      return threshold !== undefined
        ? `retried ${count}× — threshold is ${threshold}`
        : `retried ${count}×`;
    }
    ...
    case "retrieval_without_grounding": {
      const spanName = evidence.span_name as string | undefined;
      return spanName
        ? `'${spanName}' returned 0 results, LLM still invoked`
