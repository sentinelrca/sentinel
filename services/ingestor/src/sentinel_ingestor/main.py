...
async def _resolve_workspace(request: Request) -> str:
    """Extract workspace_id from Authorization header (Bearer <api-key>)."""
...
async def ingest_traces(request: Request) -> Response:
    ...
    workspace_id = await _resolve_workspace(request)
    ...
    for resource_spans in export_req.resource_spans:
        ...
        for scope_spans in resource_spans.scope_spans:
            for otel_span in scope_spans.spans:
                span = map_otel_span(otel_span, workspace_id, resource_attrs)
                trace_ids.add(span.trace_id)
                # Insert span to ClickHouse inline (fast, fire-and-forget via task)
                from sentinel_pipeline.db.clickhouse import insert_spans
                await insert_spans([span])

    # Queue process_trace for each unique trace in this batch
    for trace_id in trace_ids:
        request.app.state.celery.send_task(
            "sentinel_worker.tasks.process_trace.process_trace",
            kwargs={"workspace_id": workspace_id, "trace_id": trace_id, "workspace_tier": "free"},
        )
