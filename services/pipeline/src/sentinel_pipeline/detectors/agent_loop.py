...
class AgentLoopDetector(Detector):
    ...
    def evaluate(self, graph: FlowGraph, signals: Signals) -> list[Insight] | None:
        ...
        for agent_name, count in agent_appearances.items():
            if count >= _MIN_LOOP_COUNT:
                span_ids = [s.span_id for s in graph.nodes.values() if s.agent_name == agent_name]
                insights.append(
                    Insight(
                        workspace_id=graph.workspace_id,
                        trace_id=graph.trace_id,
                ...
                if count >= _MIN_LOOP_COUNT:
                    ...
                    insights.append(
                        Insight(
                            ...
                            recommendation=(
                                "Add a loop counter or maximum-iteration guard. "
    ...
    def _agent_names_in(self, span_ids: list[str], graph: FlowGraph) -> set[str]:
        return {
            name for sid in span_ids if sid in graph.nodes and (name := graph.nodes[sid].agent_name)
        }
    ...
    def _langgraph_node_counts(self, graph: FlowGraph) -> Counter:
        ...
        for s in graph.nodes.values():
            ...
            if not s.name or s.name.lower() in _FRAMEWORK_NAMES:
                continue
            children = [graph.nodes[cid] for cid in dg.successors(s.span_id) if cid in graph.nodes]
